document.addEventListener('DOMContentLoaded', function () {
    fetch('/api/prodotti')  // Richiesta API per ottenere i dati
...
function createRegionSelect(prodotti) {
    const categoria = [...new Set(prodotti.map(item => item.categoria))]; // Crea un set di regioni uniche
...
function displayStabilimenti(prodotti, categoriaSelezionata) {
    const filteredprodotti = prodotti.filter(item => item.categoria === categoriaSelezionata);  // Filtra per regione
    ...
    filteredprodotti.forEach(prodotti => {
        ...
        listItem.textContent = `${prodotti.nome_prodotto} - ${prodotti.info_prodotto}`;
